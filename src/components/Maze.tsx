...
const Maze: React.FC<MazeProps> = ({ maze, playerPosition, checkpoints }) => {
  const getCellClass = (cell: MazeCell, x: number, y: number) => {
    ...
    switch (cell) {
      ...
      case 2: // Checkpoint
        ...
        return `${baseClass} bg-gradient-to-br from-blue-200 to-blue-300 border border-blue-400 ${checkpoint?.completed ? 'opacity-50' : 'shadow-lg'}`;
      case 3: // Finish
  ...
  const getCellContent = (cell: MazeCell, x: number, y: number) => {
    if (playerPosition.x === x && playerPosition.y === y) {
      return (
        <div className="w-full h-full flex items-center justify-center text-xs md:text-sm animate-bounce z-10 relative">
          👨‍🎓
        </div>
      );
    }
  ...
  return (
    <div className="flex items-start justify-center min-h-screen bg-gradient-to-br from-orange-100 to-amber-200 p-4">
      <div className="bg-white rounded-3xl shadow-2xl p-4 md:p-6 mt-20 md:mt-0">
